function sendColdEmail() {
  ...
  for (var i = 3; i <= rowNum; i++) {
    ...
    if (isResponseBlank && mainSheet.getRange(i, 3).isBlank()) {
      ...
      // get contact info
      let receiverName = mainSheet.getRange(i, 9).getValue();
      let schoolName = mainSheet.getRange(i, 5).getValue();
      let emailAddress = mainSheet.getRange(i, 10).getValue();
      ...
      if (dateCheck1.isBlank()) {
        ...
        let emailSubject = generalSubjects[emailType].replace("{schoolName}", schoolName);
        let emailBody = tempBody.replace("{receiverName}", receiverName).replace(new RegExp("{senderName}", "g"), senderName);
